...
- [x] **BNS 2023 fully ingested and LIVE** (2026-07-14): all 358 sections + 20 chapters from the official Gazette PDF (MHA mirror; indiacode.nic.in was unreachable network-wide — bitstream PDF mirrored). Parser: `pdftotext -bbox` word coordinates + **font-height discrimination** (notes 8.8pt vs body 11pt) — survives column drift, kerning quirks ("CHAPTERI", "192.Whoever"), alternating margin sides, note overflow past section starts. Verbatim spot-checks: §1, §24/25 (pathological), §63, §103, §302, §304 Snatching, §358. Published over seed rows (IDs preserved → mappings intact); verified live via REST + app preview
- [ ] Remaining priority acts via the same pipeline: BNSS, BSA (same gazette format), then IPC/CrPC/IEA (older typography — expect parser calibration), Constitution, Contract Act
- [ ] Full clause-level proofread pass of BNS against the PDF (spot-verified only; provenance says so)
