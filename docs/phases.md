...
- [x] **Marginal-note wrap artifacts fixed — ~480 new-code notes corrected** (2026-07-16): the "~40 artifacts" estimate was low. Root cause: the note column's 9.6pt leading drifts into body lines' ±4pt baseline window every few lines, and the left-margin split tested word ENDS (xMax ≤ 135) — body continuation words (x≈117.6) passed it, so the split failed and note words leaked into bodies ("Sexual ~~intercourse~~ by employing deceitful" → BNS §69). Fix: column membership by word STARTS (notes ≤ 112, body resumes ≥ 115). Also: note tails that wrap below the NEXT section's start (or below a chapter heading — section flush now deferred past chapter headings) are peeled back to a ragged (period-less) previous note (BNSS §391 "…before themselves." / §392 "Judgment", §401/402); citations tolerate padding and stacking ("32 of 2012 .", "20 of 1958. 2 of 2016."); bbox parsing now ends at schedule headings like the inline parser (BNSS §531's body was 172 KB of First Schedule + forms → 1.8 KB; BSA §170 3.4 KB → 495 B) with a hardened trailing-signatory strip. **Audit oracle: NCRB table titles** — 1,050 new-code sections compared, 14 residual mismatches, every one verified against the raw PDFs as NCRB-side variants; old-code residuals are India Code's own body-heading typos kept source-faithfully (IPC §312 "miscarraige", §452 "alter preparation", CrPC §447 "insolvency of death", §409 "Session Judge" — the TOCs spell them correctly, the body headings don't). 7 new parser unit tests; republished via edge function v14 → tombstone v15; verified live over anon REST.
