...
0. **Phase 1 next**: ingest BNSS + BSA (same gazette format — fetch MHA/gazette PDFs, run parse-gazette on -bbox output); then IPC/CrPC/IEA (older typography, expect calibration); full BNS proofread pass; mapping-bundle format for the complete IPC⇄BNS table (~511 rows; authoritative source needed). Note: BNS seed rows replaced in place; IPC/CrPC/IEA/BNSS/BSA still carry dev-sample placeholders (flagged in UI) until their ingestion. (BNS ✓ 2026-07-14; web SEO pages ✓; pipeline ✓.)
